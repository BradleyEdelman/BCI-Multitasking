...
baselineTimespan = [EEG.xmin s.correctionLimits(1)];

if s.doBaselineSubtraction	
	c_say('Performing baseline subtraction');
...
for iE = 1:length(epochedEEG.epoch)
	...
	if hasMultipleEvents
		...
		eventIndex = epochedEEG.epoch(iE).eventurevent{eventSubIndex};
	else
		eventSubIndex = find(eventLatencies==0,1,'first');
