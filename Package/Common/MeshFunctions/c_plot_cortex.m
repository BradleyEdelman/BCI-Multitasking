...
p.addParameter('inputUnit',1);
p.addParameter('dispUnit',1e-6);
...
if s.doDoublePlot
	if s.doShowColorbar
		ha(1) = c_subplot('position',[0 0 0.45 1]);
	else
		ha(1) = c_subplot(1,2);
	end
	...
	if s.doShowColorbar
		ha(2) = c_subplot('position',[0.45 0 0.45 1]);
	else
		ha(2) = c_subplot(2,2);
	end
...
if c_isFieldAndNonEmpty(mesh_cortex,'SulciMap')
	c_plotSurface(mesh_cortex.Vertices,mesh_cortex.Faces,...
		'nodeData',bsxfun(@times,(2-mesh_cortex.SulciMap)/2,[0.7 0.7 0.9]),...
		plotSurfaceArgs{:});
else
	c_plotSurface(mesh_cortex.Vertices,mesh_cortex.Faces,...
		plotSurfaceArgs{:});
...
dataMultiplier = c_convertValuesFromUnitToUnit(1,s.inputUnit,s.dispUnit);
...
if ~isempty(s.data)
	...
	if isscalar(s.colorLimits)
		if s.doSymmetricLimits
			colorLimits = [-1 1]*s.colorLimits*max(s.data(:));
		else
			colorLimits = [0 1]*s.colorLimits*max(s.data(:));
		end
	else
