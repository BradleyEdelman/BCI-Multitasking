...
if (K > 1)
    ...
    if ~isParallel && (nTest < 2000) 
		c_saySingle('Using slow search'); %TODO: debug, delete
		tic
        if isProgress
	...
	elseif ~isParallel
		tic
		%====== custom kd-tree method =======
		c_say('Creating kd-tree searcher');
		searcher = createns(refVert,'NSMethod','kdtree','Distance','euclidean');
		c_sayDone();
		
		c_say('Searching');
		[I, dist] = knnsearch(searcher,testVert,'K',K);
		c_sayDone();
		
		dist = double(dist); % some later functions expect this to be double rather than single
