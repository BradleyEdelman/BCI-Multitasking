...
multiFigActions = {'tile','show','close','link'};
	...
	case 'tile'
		if length(s.monitors) > 1
			...
			if ischar(s.tileDims) && strcmpi(s.tileDims,'auto')
				numCols = min(ceil(sqrt(numFigs)*length(s.monitors)),numFigs);
			...
			% assume (for now) that all monitors are landscape
			for iF = 1:numFigs
				rowNum = ceil(iF/numCols);
				colNum = mod(iF-1,numCols)+1;
				monNum = ceil(colNum / colsPerMon);
				colNum = mod(colNum-1,colsPerMon)+1;
			...
			if ~isLandscape
				% transpose numbers of rows and columns
				tmp = numCols;
				numCols = numRows;
				numRows = tmp;
			end
			...
			for iF = 1:numFigs
				rowNum = ceil(iF/numCols);
				colNum = mod(iF-1,numCols)+1;
				pos = [(colNum-1)/numCols, (rowNum-1)/numRows, 1/numCols, 1/numRows];
				set(s.figHandles(iF),'OuterPosition',convertRelativePosToPixelPos(pos,s.monitors));
			end
		end
