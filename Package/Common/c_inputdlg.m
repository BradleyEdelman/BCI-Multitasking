...
for iF = 1:numFields
	...
	if ~isFile(iF)
		...
		if ~isempty(si.relativeToDir)
			dir = si.relativeToDir
			filename = defaultVal
		else
			[dir, filename] = fileparts(defaultVal)
		end
		h_f_filefield(iF) = c_GUI_FilepathField(...
			...
			'Position',pos);
		if s.doLiveValidation
			h_f_filefield(iF).browseCallback = ...
				@(file) revalidateField(struct('String',sprintf('''%s''',file)),...
