function [plotting,event]=bci_ESI_HandMovement2(handles,event,control,type)

paradigm=get(handles.paradigm,'value');

% DEFINE HAND RANGE OF MOTION
hitcriteria=handles.BCI.SMR.param.hitcriteria;
...
plotting=handles.Stimulus.Hand.(type);
pointstmp=plotting.vertices;

% Bring predefined vertex to origin before rotating
offsetfield=strcat(type,'offset');
pointstmp(:,1)=pointstmp(:,1)-handles.Stimulus.Hand.(offsetfield)(1);
pointstmp(:,2)=pointstmp(:,2)-handles.Stimulus.Hand.(offsetfield)(2);
