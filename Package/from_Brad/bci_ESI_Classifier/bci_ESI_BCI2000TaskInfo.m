function [handles,TaskInfo,Data]=bci_ESI_BCI2000TaskInfo(handles,TrainFiles,TrialStruct)
...
if strcmp(TrialStruct.tasktype,'Cursor')
    ...
    for j=1:size(TrainFiles,1)
        ...
        ChanOffset=TrialStruct.chanoffset;
        ChanGain=TrialStruct.changain;
        CurrentData=double(EEG.data);
        
        for i=1:size(CurrentData,1)
            CurrentData(i,:)=CurrentData(i,:)*ChanGain(i)+ChanOffset(i);
        end
...
elseif strcmp(TrialStruct.tasktype,'Stimulus')
    ...
    for j=1:size(TrainFiles,1)
        ...
        for k=1:CurrentTrial
            ...
            TaskInfo{k+1+TotTrial,5}=TaskInfo{k+1+TotTrial,4}+TrialStruct.stimdur*TrialStruct.fs-1; % Stimulus End
            TaskInfo{k+1+TotTrial,6}=TrialStruct.stimdur*TrialStruct.fs; % Stimulus Duration
        ...
        TotTrial=TotTrial+CurrentTrial; % Total trials in all data sets
        if j<size(TrainFiles,1) && isequal(TrialStruct.stimname(:,j),TrialStruct.stimname(:,j+1))
        else
