function [hObject,handles]=bci_ESI_RunTrainSSVEP(hObject,handles)
                    ...
                    if ~isempty(EEG(i).data)
                        ...
                        switch tempdomain
                            ...
                            case 2
                                ...
                                A=zeros(size(EEG(i).data,1),numtargetfreq);
                                B=zeros(size(EEG(i).data,1),numtargetfreq);
                                for j=1:size(EEG(i).data,1)
                                    for k=1:numtragetfreq
                                        % project time filtered signal to sine/cosine space
                                        % Target frequency
                                        COS1=dot(EEG(i).data(j,:),cos(2*pi*targetfreq(k)))^2;
                                        SIN1=dot(EEG(i).data(j,:),sin(2*pi*targetfreq(k)))^2;
                                        A(j,k)=sqrt(COS1+SIN1);
                                        
                                        % Second harmonic of target frequency
                                        COS2=dot(EEG(i).data(j,:),cos(2*pi*2*targetfreq(k)))^2
                                        SIN2=dot(EEG(i).data(j,:),sin(2*pi*2*targetfreq(k)))^2
                                        B(j,k)=sqrt(COS2+SIN2);
