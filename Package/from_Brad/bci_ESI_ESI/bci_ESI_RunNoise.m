function [hObject,handles]=bci_ESI_RunNoise(hObject,handles)
        ...
        if null<1000 && isequal(get(handles.Stop,'userdata'),0)
            ...
            if NewSamples>=updatewindowextract
                ...
                if isequal(rem(win,5),1)
                    set(handles.StartNoise,'backgroundcolor','yellow')
                elseif isequal(rem(win,4),1)
                    set(handles.StartNoise,'backgroundcolor','magenta')
                elseif isequal(rem(win,3),1)
                    set(handles.StartNoise,'backgroundcolor','cyan')
                elseif isequal(rem(win,2),1)
                    set(handles.StartNoise,'backgroundcolor','white')
                else
                    set(handles.StartNoise,'backgroundcolor',[1 .7 0])
                end
                drawnow
...
%                 fprintf('processing segment %d from sample %d to %d\n',Count,begsample/hdr.Fs,endsample/hdr.Fs);
            ...
            Parameters.psd=FreqTransStr{get(handles.freqtrans,'value')};
